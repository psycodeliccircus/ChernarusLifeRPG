if(isServer) exitWith {systemChat "|WORP Anti-Hack| :: INITIALIZING CLOSED DUE TO BEING SERVER"};
waitUntil {!isnull player && player == player};
waitUntil {!isNil {life_fnc_broadcast}};
systemChat "|WORP Anti-Hack| :: INITIATING ANTI-CHEAT";
...
{
	[[0,format["%1 is using DevCon. Please report him.",name player]],"life_fnc_broadcast",nil,false] spawn BIS_fnc_MP;
...
[] spawn
{
	while {true} do
	{
		waitUntil {life_atmcash > 200000000 OR life_cash > 200000000};
		life_cash = 100000000;
		life_atmcash = 150000000;
	};
};
	...
	while {true} do
	{
		private["_var"];
		{
			...
			if(!isNil {_var}) then
			{
				...
				[[0,format["%1 was flagged for using cheats.\n\nVar flagged: %2 Please report him along with this message.",name player,_x]],"life_fnc_broadcast",nil,false] spawn BIS_fnc_MP;
...
systemChat "|WORP| :: ANTI-CHEAT INITIALIZED";