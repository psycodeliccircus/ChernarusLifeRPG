...
*/
  ...
  /* Configuration */
Admin_List = compileFinal "[""76561198079682161""]";		//replace these with your admin player UIDS (steamID64)
if(!isDedicated) then {
	waitUntil {player == vehicle player && getPlayerUID player != ""}; //CL - Wait until the player has initiatied fully, maybe a fix for the admincheck error.
};
/* End Configuration */
...
if(isServer) then {
	...
	AH_Menu_TPPHere = {
		...
		if(_object call AH_AdminCheck) then {
			...
			format["%1 has teleported %2 to his position!",name _object ,name _target] call SERVER_LOG;
			[{hint "Player has been teleported to you!";},"BIS_fnc_Spawn",_object,false] call AH_fnc_MP;
		};
	};
	AH_Menu_Kick = {
	...
	AH_Menu_TPPHere = compileFinal ([AH_Menu_TPPHere] call _toCompilableString);
	...
	if(player call AH_AdminCheck) then {
		...
		if(isNil "AH_Init") then {
			[] spawn {
				while{true} do {
					...
					AH_BIND = (findDisplay 46) displayAddEventHandler ["KeyDown",{if((_this select 1) == 61) then {[] spawn AH_ViewLogs;};if((_this select 1) == 59) then {if( (cursorTarget iskindof "Air" ) || (cursorTarget iskindof "Tank") || (cursorTarget isKindOf "Land") || (cursorTarget isKindOf "Ship")) then {deleteVehicle cursorTarget;} else {hint "Cannot Delete Target.";};};if((_this select 1) == 60) then {cursorTarget setDamage 0;cursorTarget setFuel 1;cursorTarget setVectorUp (surfaceNormal (getpos cursorTarget));};false}];
				};
			};
			[] spawn {
				while{true} do {
		...
		AH_Init = {
			if(player call AH_AdminCheck) then {
				if(isNull (FindDisplay 163)) then {
					...
					if(AH_GM) then {
						_ctrl lbSetColor [5,[0,1,0,1]];
					} else {
						_ctrl lbSetColor [5,[1,0,0,1]];
					};
		...
		AH_Target = {
			...
			if(_script == 4) then {
				...
				_ctrl buttonSetAction "[[player,(lbText[101,lbCurSel 101] call AH_GetObject),true],'AH_Menu_TPPHere',false,false] call AH_fnc_MP";
				_ctrl ctrlCommit 0;
			};
			_ctrl = (findDisplay 164) displayctrl 101;
			{
			...
			lbSort _ctrl; //NDJ 28Jun14 Sort the list alphabetically
			_ctrl ctrlCommit 0;
		};
		AH_DBLClick = {
			if !(player call AH_AdminCheck) exitWith {};
			...
			switch(_index) do {
				...
				case 4: {4 spawn AH_Target;};
				case 5: {if(player call AH_AdminCheck) then {AH_GM = !AH_GM; if(AH_GM) then {lbSetColor[101,5,[0,1,0,1]];player allowDamage false;[] spawn AH_CarGod;hint "God Mode ON";} else {lbSetColor[101,5,[1,0,0,1]];player allowDamage true;[] spawn AH_CarGod;hint "God Mode OFF";};};};
				case 6: {[] spawn AH_MapMarkers;};
				case 7: {[] spawn AH_VehMarkers;};
				case 8: {call AH_ESP;};
			};
		};
		AH_CarGod = {
		...
		AH_TP = {
			if(player call AH_AdminCheck) then {
				...
				openMap[true,false];
				onMapSingleClick '[[player,_pos],"AH_Menu_DoTP",false,false] call AH_fnc_MP;openMap[false,false];onMapSingleClick "";false';
			};
		};
		...
		hint parseText format["Press '%1' to open the admin menu!<br/>Press F1 F2 and F3 to delete or open the log menu respectively",(actionKeysNames ["moveRight",1])];
	};
};