...
switch (playerSide) do
{
	case civilian:
	{
       ...
       case west:
       {
                ...
                //Seize weapons
                life_actions = life_actions + [player addAction["<t color='#FFFF00'>apreensão de armas</t>+",life_fnc_seizePlayerWeapon,cursorTarget,0,false,false,"",'!isNull cursorTarget && (player distance cursorTarget) < 6 && speed cursorTarget < 2 && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget == civilian) && (cursorTarget getVariable "restrained")']];
        
                //Seize Objects
                life_actions = life_actions + [player addAction["<t color='#FFFF00'>apreensão de objetos</t>",life_fnc_seizeObjects,cursorTarget,0,false,false,"",'count(nearestObjects [player,["weaponholder"],3])>0']];
       }
};
